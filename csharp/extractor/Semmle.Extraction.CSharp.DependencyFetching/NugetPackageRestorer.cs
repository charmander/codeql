...
namespace Semmle.Extraction.CSharp.DependencyFetching
{
    internal sealed partial class NugetPackageRestorer : IDisposable
    {
        ...
        private (HashSet<NugetConfig.NugetFeed> explicitFeeds, HashSet<NugetConfig.NugetFeed> allFeeds) GetAllFeeds()
        {
            ...
            {
                var proxyFeed = new NugetConfig.NugetFeed($"https://{proxyHost}:{proxyPort}/index.json");
                proxyFeed.DisableTlsCertificateValidation = true;
                logger.LogInfo($"Added private registry proxy feed at {proxyFeed}");
                explicitFeeds.Add(proxyFeed);
            }
